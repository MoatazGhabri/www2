...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $admin_property_count = Property::whereHas('user', function ($query) {
            $query->where('is_admin', 1);
        })->count();

        // Count vedette (premium) properties
        $vedette_count = \App\Models\Property_features::where('type__', 'property')->count();

        // Count latest properties (last 7 days)
        $latestCount = Property::whereHas('user', function ($query) {
            $query->where('is_admin', 1);
        })->where('created_at', '>=', now()->subDays(7))->count();

        // Count all ServiceWeb entries
        $serviceWebCount = \App\Models\ServiceWeb::count();

        // Message statistics
        $messagesReceived = \App\Models\AdminMessage::count();
        $messagesReplied = \App\Models\AdminMessage::where('status', 'replied')->count();
        $messagesRead = \App\Models\AdminMessage::where('status', 'read')->count();

        // Messages received today
        $messagesReceivedToday = \App\Models\AdminMessage::whereDate('created_at', now()->toDateString())->count();

        return view("dashboard_admin.index", compact('admin_property_count', 'vedette_count', 'latestCount', 'serviceWebCount', 'messagesReceived', 'messagesReplied', 'messagesRead', 'messagesReceivedToday'));
    }

    public function get_admin_properties(Request $request)
    {
        try {
            $user = auth()->user();
            if (!$user->isAdmin()) {
            ...
            // Add: Count of latest properties (last 7 days)
            $latestCount = Property::where('user_id', $user->id)
                ->where('created_at', '>=', now()->subDays(7))
                ->count();

            return view("dashboard_admin.all_annonces.index", compact("props", "categories", "featuresExist", "latestCount"));
        } catch (\Throwable $e) {
            return redirect()->back();
        }
    }
    ...
    public function updateProperty(PropertyRequest $request, Property $property)
    {
        try {
            ...
            // Always redirect to all properties after update
            return redirect()->route('admin_all_properties')->with('success', 'Propriété modifiée avec succès');
        } catch (\Exception $e) {
...
// dd("eee");
    ...
    {
        $messages = AdminMessage::orderBy('created_at', 'desc')->paginate(15);
        $unreadCount = AdminMessage::where('status', 'unread')->count();
        return view('dashboard_admin.messages.index', compact('messages', 'unreadCount'));
    }

    public function deleteMessage(AdminMessage $message)
    {
        $message->delete();
    ...
    public function showAdminMessage(AdminMessage $message)
    {
        return view('dashboard_admin.messages.show', compact('message'));
    }

    public function markAdminMessageRead(Request $request, AdminMessage $message)
    {
        $message->status = 'read';
        $message->read_at = now();
        $message->save();
    ...
    public function markAdminMessageReplied(Request $request, AdminMessage $message)
    {
