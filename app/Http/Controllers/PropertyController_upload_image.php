...
class PropertyController extends Controller
{
    ...
    private function resizeImage($image, $path, $width, $height)
    {
        ...
        if (!in_array($extension, ['gif', 'png', 'jpg', 'jpeg', 'webp'])) {
            throw new Exception('Format d\'image non pris en charge'); // Handle unsupported formats
        }

        $imageResource = imagecreatefromstring($image->getContent());
        ...
        imagecopyresampled($thumb, $imageResource, $offsetX, $offsetY, 0, 0, $newWidth, $newHeight, $sourceWidth, $sourceHeight);
...
// dd($property);
    ...
    public function uploaded (Request $request,Property $property){
        ...
        $files = $request->file('photos_multiple');
        \Log::info('photos_multiple received', [
            'type' => gettype($files),
        ...
        if (!$files || (is_array($files) && count($files) == 0)) {
            return redirect()->back()->withErrors(['propertyError' => 'Aucune image de bien n\'a été reçue. Veuillez sélectionner au moins une image.']);
        }
        try {
            $this->updateMainPicture($request, $property);
        } catch (\Exception $e) {
