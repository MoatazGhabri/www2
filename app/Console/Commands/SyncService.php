...
Log::info('Status code register user service' . $statusCodeRegister);
                        ...
                        if ($user_id !== "") {
                            ...
                            if ($service->synced_premier) {

                                try {

                                    $serviceResponsePremier = $client->request('POST', $premierUrl . 'api/add/service', [
                                        ...
                                        'multipart' => $serviceData, // reuse the same serviceData array

                                        'verify' => false,

                                    ]);

                                    $statusCodePremier = $serviceResponsePremier->getStatusCode();

                                    $responseBodyPremier = $serviceResponsePremier->getBody()->getContents();

                                    Log::info('Status Code Immobilier.tn: ' . $statusCodePremier);

                                    Log::info('Response Body Immobilier.tn: ' . $responseBodyPremier);

                                } catch (\Exception $e) {

                                    Log::error('Error for Immobilier.tn API service: ' . $e->getMessage());
                            ...
                            try {
                                ...
                                $serviceResponseImmobiliere = $client->request('POST', $immobiliere_url . 'api/add/service', [
                                ...
                                $statusCodeImmobiliere = $serviceResponseImmobiliere->getStatusCode();

                                $responseBodyImmobiliere = $serviceResponseImmobiliere->getBody()->getContents();
                                ...
                                Log::error('Error for immobiliere.tn API service: ' . $e->getMessage());
