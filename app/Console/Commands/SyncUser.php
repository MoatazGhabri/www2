...
class SyncUser extends Command
{
    ...
    public function handle()
    {
        ...
        $apiKeyPremier = env('API_KEY_ONE');
        $premierUrl = env('PREMIER_SITE_LINK');
        ...
        if (count($properties) > 0) {
            foreach ($properties as  $property) {
                ...
                switch ($category) {
                    ...
                    case 'admin':
                        Log::info('start admin:::admin');
                        // Pour les admins, on peut utiliser les données de base de l'utilisateur
                        $userType = $user;
                        Log::info('end admin:::admin ' . $userType->id);
                        break;
                ...
                try {
                    ...
                    if ($statusCode == 200) {
                        ...
                        if ($dataCheckUser['exists'] == false) {
                            $userResponse = $client->request('POST', $prod_url . 'api/user/register', [
                                ...
                                'form_params' => [
                                    'name' => $user->username,
                                    'email' => $email,
                                    'category' => ($category === 'admin') ? 'company' : $category,
                            ...
                            if ($dataCheckUser['exists'] == true) {
                                ...
                                // Removed logic that creates a new user with a random email for admin if user exists
                            } else {
                        ...
                        if ($user_id !== "") {
                            ...
                            // New: Sync to Immobilier.tn if synced_premier is true
                            if ($property->synced_premier) {
                                try {
                                    // Check user existence on immobiliere.tn
                                    $urlImmobiliere = $premierUrl . 'api/users/check?email=' . urlencode($email);
                                    Log::info('Checking user on immobiliere.tn: ' . $email);
                                    
                                    $responseImmobiliere = $client->request('GET', $urlImmobiliere, [
                                        'headers' => [
                                            'X-API-KEY' => $apiKeyPremier,
                                        ],
                                        'verify' => false,
                                    ]);
                                    
                                    $statusCodeImmobiliere = $responseImmobiliere->getStatusCode();
                                    $responseBodyImmobiliere = $responseImmobiliere->getBody()->getContents();
                                    $dataCheckUserImmobiliere = json_decode($responseBodyImmobiliere, true);
                                    
                                    Log::info('Response from immobiliere.tn API for check email: ' . $responseBodyImmobiliere);
                                    
                                    if ($statusCodeImmobiliere == 200) {
                                        if ($dataCheckUserImmobiliere['exists'] == false) {
                                            // User doesn't exist, register them
                                            $userResponseImmobiliere = $client->request('POST', $premierUrl . 'api/user/register', [
                                                'headers' => [
                                                    'X-API-KEY' => $apiKeyPremier,
                                                ],
                                                'form_params' => [
                                            ...
                                            $responseUserBodyImmobiliere = $userResponseImmobiliere->getBody()->getContents();
                                            $dataRegisterUserImmobiliere = json_decode($responseUserBodyImmobiliere, true);
                                            Log::info('Response from Create User on immobiliere.tn: ' . $responseUserBodyImmobiliere);
                                            
                                            $user_id_immobiliere = $dataRegisterUserImmobiliere['index'];
                                        } else {
                                            // User exists, use existing user ID
                                            $user_id_immobiliere = $dataCheckUserImmobiliere['index'];
                                            // Removed logic that creates a new user with a random email for admin if user exists
                                        }
                                        
                                        // Now sync property to immobiliere.tn with correct user_id
                                        if ($user_id_immobiliere !== "") {
                                            // Recreate propertyData for immobiliere.tn
                                            $propertyDataImmobiliere = [
                                                [
                                                    'name' => 'token',
                                                    'contents' => $property->token,
                                                ],
                                            ...
                                            // Re-add images for immobiliere.tn
                                            if ($property->main_picture) {
                                                $mainPicturePath = public_path('uploads/main_picture/images/properties/' . $property->main_picture->alt);
                                                if (file_exists($mainPicturePath)) {
                                                    $propertyDataImmobiliere[] = [
                                            ...
                                            $propertyResponseImmobiliere = $client->request('POST', $premierUrl . 'api/add/property', [
                                                'headers' => [
                                                    'X-API-KEY' => $apiKeyPremier,
                                                ],
                                                'multipart' => $propertyDataImmobiliere,
                                                'verify' => false,
                                            ]);
                                            $statusCodeImmobiliere = $propertyResponseImmobiliere->getStatusCode();
                                            $responseBodyImmobiliere = $propertyResponseImmobiliere->getBody()->getContents();
                                            Log::info('Status Code Immobilier.tn: ' . $statusCodeImmobiliere);
                                            Log::info('Response Body Immobilier.tn: ' . $responseBodyImmobiliere);
                                        } else {
                                            Log::error('Error for immobiliere.tn: user_id is empty');
                                        }
                                    } else {
                                        Log::error('Error checking user on immobiliere.tn: ' . $statusCodeImmobiliere);
                                    }
                                } catch (\Exception $e) {
                                    Log::error('Error for Immobilier.tn API property: ' . $e->getMessage());
                                }
                            }

                            // Remove old immobiliere.tn sync code
                            // $immobiliere_url = "https://immobiliere.tn/";
                            // $immobiliere_api_key = env('IMMOBILIERE_API_KEY');
                            // try {
                            //     $propertyResponseImmobiliere = $client->request('POST', $immobiliere_url . 'api/add/property', [
                            //         'headers' => [
                            //             'X-API-KEY' => $immobiliere_api_key,
                            //         ],
                            //         'multipart' => $propertyData, // reuse the same propertyData array
                            //         'verify' => false,
                            //     ]);
                            //     $statusCodeImmobiliere = $propertyResponseImmobiliere->getStatusCode();
                            //     $responseBodyImmobiliere = $propertyResponseImmobiliere->getBody()->getContents();
                            //     Log::info('Status Code immobiliere.tn: ' . $statusCodeImmobiliere);
                            //     Log::info('Response Body immobiliere.tn: ' . $responseBodyImmobiliere);
                            // } catch (\Exception $e) {
                            //     Log::error('Error for immobiliere.tn API property: ' . $e->getMessage());
                            // }
                        } else {
