...
<div class="serviceweb-add-card">
    <div class="serviceweb-add-title">Ajouter un Service Web</div>
    <form action="{{ route('admin.store_service_web') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="serviceweb-form-group">
            <label for="title">Titre</label>
            <input type="text" class="@error('title') is-invalid @enderror" placeholder="Entrez le titre" name="title" id="title" value="{{ old('title') }}" maxlength="34">
        </div>
        <div class="serviceweb-form-group">
            <label for="description">Description</label>
            <textarea class="@error('description') is-invalid @enderror" placeholder="Entrez la description" name="description" id="description">{{ old('description') }}</textarea>
        </div>
        <div class="serviceweb-form-group">
            <label for="lien">Lien</label>
            <input type="text" placeholder="Entrez le lien" name="lien" id="lien" value="{{ old('lien') }}">
        </div>
        <div class="serviceweb-form-group">
            ...
            <img id="mainPicturePreviewImg" class="serviceweb-img-preview" src="#" alt="Aperçu de l'image" style="display:none;">
        </div>
        ...
        <button type="submit" class="serviceweb-submit-btn">Enregistrer</button>
    </form>
</div>
<script>
