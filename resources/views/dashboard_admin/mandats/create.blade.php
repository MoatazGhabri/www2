...
<div class="mandat-modern-card">
    ...
    <div class="mandat-modern-title">Ajouter un mandat</div>
    <form action="{{ route('admin.mandats.store') }}" method="POST">
        @csrf
        ...
        <div class="mandat-form-group">
            ...
            <select name="location" id="location" required>
                ...
                @foreach($cities as $city)
                    <option value="{{ $city->name }}" {{ old('location') == $city->name ? 'selected' : '' }}>{{ $city->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="mandat-form-group">
        ...
        <div class="mandat-form-group">
            ...
            <textarea name="observations" id="observations" rows="2">{{ old('observations') }}</textarea>
        </div>
        <button type="submit" class="mandat-submit-btn">Enregistrer</button>
    </form>
</div>
@endsection 