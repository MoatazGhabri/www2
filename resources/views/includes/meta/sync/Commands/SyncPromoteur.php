...
class SyncPromoteur extends Command
{
    ...
    {
        $properties = PromoteurProperty::where('synced', true)->where('status', 1)->orderBy("created_at","asc")->limit(1)->get();
...
Log::info('Status code check user promoteur' . $statusCodeRegister);
                    // dd($statusCode);
                    $responseBody = $response->getBody()->getContents();

                    $dataCheckUser = json_decode($responseBody, true);
                    // dd($dataCheckUser);
                    Log::info('Response from API for check email Promoteur ' . $email . ': ' . $responseBody);
...
Log::info('status code create user Promoteur ' . $statusCodeRegister );

                            $responseUserBody = $userResponse->getBody()->getContents();
                            // dd($responseUserBody);
                            $dataRegisterUser = json_decode($responseUserBody, true);
                            Log::info('Response from API Create User Promoteur'  . $dataRegisterUser);
                            ...
                            if ($dataCheckUser['exists'] == true) {
                                ...
                                Log::info('Response user Promoteur id service problem: ' . $dataCheckUser);
                                dd($dataCheckUser);
                            }
                        }
                        ...
                        if ($user_id !== "") {
                            ...
                            Log::info('Response Body create property promoteur: ' . $responseBodyproperty);
